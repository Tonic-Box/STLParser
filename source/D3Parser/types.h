...
typedef struct {
	...
	int32_t unknown;
	int32_t unused2;
	MPQGamDataArrayEntry dataArrayEntry[35];	// array of DataArrayEntry (see below)
} MPQGamHeader;
...
typedef struct {
	...
	int32_t unknown1[13];	// ? (not unique so it's not an id)
	...
	int32_t unknown4;	// ? (values range from 0 to 3F)
	...
	int32_t unknown7;	// hash? (only populated for Legendary Mana items)
	int32_t unused2[5];	// always 0xFFFFFFFF
	int32_t itemTypes[24];	// item group hashes which can have this affix (up to32)
	int32_t legendaryItemTypes[24];	// item group hashes which can have this affix (up to32)
...
typedef struct {
	...
	int32_t unknown[2];	// always 0x00000000
...
typedef struct {
	...
	int32_t unused2[3];	// always 0x00000000
...
typedef struct {
	...
	int32_t unknown6[4];	// related to level?
	...
	int32_t unknown10A[8];	// ???
	//int32_t unknown11;	// ???
	//int32_t unknown12[26];	// ???
	...
	int32_t unknown13[20];	// 0x00000000 (added in Beta 7728)
	...
	int32_t unknown17[47];	// always 0x00000000
	int32_t unknown19[16];	// always 0x00000000
} MPQItem;
